...
#if 0  //why add this part is very hard to connect
...
{
    light_driver_init();				//timer init
    //light_cwrgb_driver_init();	    //cwrgb light pin init

	board_plug_io_init();				//Plug control pin init
	light_pwmControl_driver_init(); 	//plug led pin config
...
void app_enter_dlps_config(void)
{
	...
	plug_io_on_off_pin_enter_dlps_config();

	plug_light_enter_dlps_config();
}
...
{
	plug_io_on_off_pin_exit_dlps_config();

	plug_light_exit_dlps_config();
