...
void light_pwmControl_driver_init(void)
{
    for (uint8_t i = 0; i < sizeof(light_PwmControl) / sizeof(light_t); ++i)
    {
        light_pin_config(&light_PwmControl[i]);
    }

	light_set_all_turn_on(65535);
}

void light_blink_ledPwm(uint32_t hz_numerator, uint32_t hz_denominator, uint8_t duty)
{
    //DBG_DIRECT("===enter light_blink_ledPwm");
    light_blink_infinite(&light_PwmControl[0], hz_numerator, hz_denominator, duty);
}

void light_set_all_turn_on(uint16_t lightness)
{
	//DBG_DIRECT("===light_set_all_turn_on");
    light_set_lightness(&light_PwmControl[0], lightness);
}

void light_set_all_turn_off(void)
{
	//DBG_DIRECT("===light_set_all_turn_off");
    light_set_lightness(&light_PwmControl[0], 0);
}

void light_set_enter_dlps(void)
{
    for (uint8_t i = 0; i < sizeof(light_PwmControl) / sizeof(light_t); ++i)
    {
        light_pin_dlps_config(&light_PwmControl[i]);
    }
}

void plug_light_enter_dlps_config(void)
{
#if 1
...
void plug_light_exit_dlps_config(void)
{
	//此时的灯配置的是pwm脚，不能直接恢复成pinmux mode
}
