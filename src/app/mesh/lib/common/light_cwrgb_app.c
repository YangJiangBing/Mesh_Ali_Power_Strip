...
/**  * @brief  Initialization of plug pinmux settings and pad settings.  
	 * @param  No parameter.
     * @return void
*/
void board_plug_io_init(void)
{
	//add switch on off pin	
	Pad_Config(LEFT_PLUG_ON_OFF_PIN, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,PAD_OUT_LOW);	   	
	Pinmux_Config(LEFT_PLUG_ON_OFF_PIN, DWGPIO);

	RCC_PeriphClockCmd(APBPeriph_GPIO, APBPeriph_GPIO_CLOCK, ENABLE);   
	GPIO_InitTypeDef GPIO_InitStruct;   
	
	GPIO_StructInit(&GPIO_InitStruct);
	GPIO_InitStruct.GPIO_Pin                = GPIO_GetPin(LEFT_PLUG_ON_OFF_PIN);  
	GPIO_InitStruct.GPIO_Mode               = GPIO_Mode_OUT;
    GPIO_InitStruct.GPIO_ITCmd              = DISABLE;    
	GPIO_Init(&GPIO_InitStruct);    
	GPIO_ResetBits(GPIO_GetPin(LEFT_PLUG_ON_OFF_PIN));
}

/**  * @brief  Initialization of plug pinmux settings and pad settings.  
	 * @param  bool true and false
     * @return void
*/
void plug_on_off_pin_cmd(bool is_on)
{	
	if (is_on)	
	{		
		GPIO_WriteBit(GPIO_GetPin(LEFT_PLUG_ON_OFF_PIN), Bit_SET);		
		switch_status.switch_status_bit.left_switch_status_bit = 1;	
	}	
	else	
	{		
		GPIO_WriteBit(GPIO_GetPin(LEFT_PLUG_ON_OFF_PIN), Bit_RESET);	
		switch_status.switch_status_bit.left_switch_status_bit = 0;	
	}
}

/**  * @brief  function plug enter dlps 
	 * @param  void
     * @return void
*/
void plug_io_on_off_pin_enter_dlps_config(void)
{   
	if (switch_status.switch_status_bit.left_switch_status_bit)   
	{        
		Pad_Config(LEFT_PLUG_ON_OFF_PIN, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE, PAD_OUT_HIGH);   
	}   
	else   
	{       
		Pad_Config(LEFT_PLUG_ON_OFF_PIN, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE, PAD_OUT_LOW);    
	}
}

/**  * @brief  function plug exit dlps 
	 * @param  void
     * @return void
*/
void plug_io_on_off_pin_exit_dlps_config(void)
{   
	if (switch_status.switch_status_bit.left_switch_status_bit)  
	{        
		Pad_Config(LEFT_PLUG_ON_OFF_PIN, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,PAD_OUT_HIGH);    
	}   
	else    
	{        
		Pad_Config(LEFT_PLUG_ON_OFF_PIN, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE,PAD_OUT_LOW);    
